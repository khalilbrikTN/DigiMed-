...
class PatientView:
    ...
    def create_patient(request):
        ...
        if request.method == 'POST':
            ...
            if patient_data.get('password') != patient_data.get('confirm_password'):
                messages.error(request, "Passwords do not match.")
                return redirect('create_patient')

            # Hash the password
            hashed_password = make_password(patient_data.get('password'))

            try:
                # Combine user_fields and patient_fields, adding the hashed password
                combined_data = {**user_fields, **patient_fields, 'password': hashed_password}

                # Create the Patient instance
                patient = Patient.objects.create(**combined_data)

                return JsonResponse({'message': 'Patient created successfully', 'patient': patient.first_name})
            except Exception as e:
                return JsonResponse({'error': str(e)}, status=400)

    def retrieve_patient(request, nat_id):
        try:
    ...
    def update_patient(request, nat_id):
        if request.method == 'POST':
    ...
    def delete_patient(request, nat_id):
        try:
...
class DoctorView:
    ...
    @staticmethod
    def retrieve_doctor(request, nat_id):
        try:
    ...
    @staticmethod
    def update_doctor(request, nat_id):
        if request.method == 'POST':
    ...
    @staticmethod
    def delete_doctor(request, nat_id):
        try:
