...
import { BrowserRouter as Router, Route, Routes, Link, useNavigate, useLocation,useHistory  } from 'react-router-dom';
...
function SignupScreen() {
  ...
  const [password, setPassword] = useState('');
  const [errorMessage, setErrorMessage] = useState('');  // This is where the error state is declared.
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/v1.0/dbms/create-new-User/', signupData, {
        headers: {
      ...
      if (response.status === 201) {
        // On successful signup, navigate to the profile screen and pass natID
        navigate('/manage-profile', { state: { natID: NationalID } });
      } else {
        // Handle error cases such as invalid data or server issues
        setErrorMessage(response.data.detail || 'Signup failed. Please try again.');
      }
    } catch (error) {
...
const ManageProfileScreen = () => {
  ...
  const location = useLocation();
  const natID = "12345678901234"; 
  // Fetch user data with Axios
  useEffect(() => {
    const domain = "http://localhost:8000"; // Replace with the correct domain

    const fetchUserData = async () => {
      try {
        console.log("useEffect triggered with natID:", natID);
        console.log("Fetching user data...");
        
        const response = await axios.get(`${domain}/api/v1.0/dbms/User/${natID}`, {
          headers: {
            "Content-Type": "application/json",
          },
        });
        ...
        if (response.status === 200 && response.data[natID]) {
          const userData = response.data[natID];
    ...
    fetchUserData();
  }, [natID]); // Dependency array, triggers on natID change
  ...
  return (
    <div className="container">
      ...
      {/* Input fields populated with fetched data */}
      <input
        className="input"
        placeholder="First Name"
        value={firstName}
